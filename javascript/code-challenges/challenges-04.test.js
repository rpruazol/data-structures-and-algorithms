...
const sortMeetingsByDay = (arr) => {
  // Solution code here...
  // How do we know what the correct days of the week are?
  const daysOfTheWeekArr = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];
  return arr.sort((a, b) => {
    let aDayValue = null;
    let bDayValue = null;

    daysOfTheWeekArr.forEach((day, idx) => {
      if(day === a.dayOfWeek){aDayValue = idx;}
      if(day === b.dayOfWeek){bDayValue = idx;}
    });
    return aDayValue > bDayValue ? 1 : -1;
  });
};
